...
// ============================================================
//  Response Cleaner: 清理 AI 回覆中的元資料和冗餘內容
//  在送到 Telegram 之前強制過濾，不依賴 AI 是否聽話
// ============================================================

let lastAiResponse = '';

function cleanResponseForTelegram(text) {
    let cleaned = text;

    // 1. 剝除 cci: 內部連結 — [text](cci:...) → text
    cleaned = cleaned.replace(/\[([^\]]*)\]\(cci:[^)]*\)/g, '$1');

    // 2. 剝除殘留的 REMEMBER: / RECALL: 標記（extractMemoryMarkers 可能漏掉的）
    cleaned = cleaned.replace(/REMEMBER:\s*(?:\[[^\]]*\])?\s*.+?(?:\n|$)/g, '');
    cleaned = cleaned.replace(/RECALL:\s*.+?(?:\n|$)/g, '');

    // 3. 剝除 [歷史回顧] 區塊
    cleaned = cleaned.replace(/\n*\[歷史回顧\][\s\S]*?(?=\n\n|\n*$)/g, '');

    // 3.5 剝除洩漏的內部標記
    cleaned = cleaned.replace(/asVoice:\s*(true|false)\s*\n?/gi, '');
    cleaned = cleaned.replace(/^MEDIA:\s*.+$/gm, '');

    // 3.6 剝除 AI 自我介紹廢話（OpenClaw 人格系統常產生的）
    // 「我是XXX，你的YYY」開頭的自我介紹句
    cleaned = cleaned.replace(/^我是\*?\*?[^，。\n]{1,20}\*?\*?[，,]\s*你的.{0,50}[。\.]\s*\n?/gm, '');
    // 「我已經準備好」「我已經載入」類開頭
    cleaned = cleaned.replace(/^我已經(?:準備好|載入|讀完).{0,80}[。\.]\s*\n?/gm, '');
    // 引號格言（「XXX」開頭獨立行）
    cleaned = cleaned.replace(/^\*?\*?「[^」]{5,60}」\*?\*?\s*\n?/gm, '');
    // 「主人，」「老闆，」開頭的多餘招呼
    cleaned = cleaned.replace(/^(?:主人|老闆)[，,]\s*/gm, '');
    // 編號能力清單（1. **XXX**：... 連續 3 行以上）
    cleaned = cleaned.replace(/(?:^[0-9]+\.\s+\*\*[^*]+\*\*[：:].+\n){3,}/gm, '');

    // 4. 清理連續空行（最多保留一個空行）
    cleaned = cleaned.replace(/\n{3,}/g, '\n\n');

    // 5. 重複偵測（保守模式：只在極度重複時才砍，且保底 100 字）
    if (lastAiResponse && lastAiResponse.length > 200) {
        const lines = cleaned.split('\n');
        const lastLines = new Set(lastAiResponse.split('\n').filter(l => l.trim().length > 20));
        if (lastLines.size > 0) {
            const uniqueLines = lines.filter(l => !lastLines.has(l) || l.trim().length <= 20);
            const removedCount = lines.length - uniqueLines.length;
            // 超過 50% 重複才砍（從 30% 提高到 50%）
            if (removedCount > lines.length * 0.5 && removedCount > 5) {
                const dedupResult = uniqueLines.join('\n').trim();
                // 保底：砍完至少要剩 100 字，否則不砍
                if (dedupResult.length >= 100) {
                    cleaned = dedupResult;
                    console.log(`  🧹 Dedup: removed ${removedCount} repeated lines`);
                } else {
                    console.log(`  🧹 Dedup: skipped (result too short: ${dedupResult.length} chars)`);
                }
            }
        }
    }

    // 6. 回覆長度限制（可透過 .env MAX_RESPONSE_LENGTH 調整，預設 600）
    const maxLen = parseInt(process.env.MAX_RESPONSE_LENGTH || '600', 10);
    if (maxLen > 0 && cleaned.length > maxLen) {
        ...
        cleaned = lastBreak > maxLen * 0.5
            ? truncated.substring(0, lastBreak + 1)
            : truncated + '…';
        cleaned += '\n\n✂️ (回覆過長已精簡，如需完整內容請說「繼續」)';
        console.log(`  ✂️ Truncated: ${text.length} → ${cleaned.length} chars`);
    }
...
export function startTelegramBot(config, rpc, reconnectFn) {
    ...
    async function processQueue() {
        ...
        try {
            ...
            const { media, asVoice, cleanText: rawCleanText } = extractMedia(processedResponse);

            // === 強制清理：剝除元資料、限制長度、去重複 ===
            const cleanText = rawCleanText ? cleanResponseForTelegram(rawCleanText) : '';
