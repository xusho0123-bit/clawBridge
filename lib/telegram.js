    ...
    bot.on('message', async (msg) => {
        ...
        if (text === '/model') {
            const models = [
                ...
                { text: 'Claude Sonnet', callback_data: 'model_claude-3-5-sonnet' },
                { text: 'Claude Opus', callback_data: 'model_claude-3-5-opus' },
                { text: 'Gemini Pro', callback_data: 'model_gemini-2.0-pro' },
            ];
            bot.sendMessage(chatId, `🤖 目前模型: ${config.model}\n選擇新模型：`, {
                reply_markup: {
                    inline_keyboard: [
                        models.slice(0, 2).map(m => ({ text: m.text + (config.model === m.callback_data.replace('model_', '') ? ' ✓' : ''), callback_data: m.callback_data })),
                        models.slice(2, 4).map(m => ({ text: m.text + (config.model === m.callback_data.replace('model_', '') ? ' ✓' : ''), callback_data: m.callback_data })),
                    ],
                },
            });
            return;
