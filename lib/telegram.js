...
function cleanResponseForTelegram(text) {
    ...
    // 5. 重複偵測：如果跟上次回覆有大量重複行，移除重複部分
    if (lastAiResponse && lastAiResponse.length > 100) {
        const lines = cleaned.split('\n');
        const lastLines = new Set(lastAiResponse.split('\n').filter(l => l.trim().length > 15));
        if (lastLines.size > 0) {
            const uniqueLines = lines.filter(l => !lastLines.has(l) || l.trim().length <= 15);
            const removedCount = lines.length - uniqueLines.length;
            // 如果超過 30% 的行是重複的，清除重複
            if (removedCount > lines.length * 0.3 && removedCount > 3) {
                cleaned = uniqueLines.join('\n').trim();
                console.log(`  🧹 Dedup: removed ${removedCount} repeated lines`);
                if (cleaned.length < 10) {
                    cleaned = '（內容與上次重複，請換個方式提問或用 /newchat 開新對話）';
                }
            }
        }
    }
