...
export function startTelegramBot(config, rpc, reconnectFn) {
    ...
    let cascadeTurnCount = 0;          // 追蹤對話輪數
    const MAX_TURNS_BEFORE_RESET = 15; // 超過 15 輪自動開新對話（防止回覆越來越長）
    ...
    async function processQueue() {
        ...
        // 自動重置過長的對話（防止 AI 回覆越來越長）
        cascadeTurnCount++;
        if (cascadeTurnCount > MAX_TURNS_BEFORE_RESET) {
            console.log(`  ♻️ Auto-reset: ${cascadeTurnCount} turns exceeded limit`);
            rpc.resetCascade();
            cascadeTurnCount = 1;
        }
