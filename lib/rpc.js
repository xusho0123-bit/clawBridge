        ...
        res.on('data', chunk => {
            ...
            while (buffer.length >= 5) {
                ...
                if (flags & 0x02) { // end-of-stream
                    if (inactivityTimer) clearTimeout(inactivityTimer);
                    // Parse error from end-of-stream frame (e.g. "reactive state is disabled")
                    let streamErr = null;
                    if (payloadStr) {
                        try {
                            const errObj = JSON.parse(payloadStr);
                            if (errObj.error) {
                                const msg = errObj.error.message || errObj.error.code || 'unknown stream error';
                                streamErr = new Error(msg);
                                console.error(`  [stream] Server error: ${msg}`);
                            }
                        } catch { /* not JSON, ignore */ }
                    }
                    if (onEnd) onEnd(streamErr);
                    return;
                }
...
function createPollingFallback(port, csrfToken, getCascadeId) {
    ...
    async function pollResponse(timeoutMs, onUpdate, preExecutorCount, preResponseText, onPermission = null, abortSignal = null) {
        ...
        while (true) {
            ...
            if (abortSignal?.aborted) {
                console.log(`  [poll] Cancelled by user`);
...
// ============================================================
//  StreamAgentStateUpdates — 新串流方式（取代已禁用的 ReactiveUpdates）
//  2026-03-14 逆向發現，不受 reactive state 限制
// ============================================================

function createAgentStream(port, csrfToken, getCascadeId) {
    function agentStreamResponse(timeoutMs, onUpdate, onPermission, messageReadyPromise = null, abortSignal = null) {
        return new Promise((resolve, reject) => {
            let responseText = '';
            let done = false;
            let messageIsSent = !messageReadyPromise;
            let permissionPending = false;
            let lastStatus = '';
            let plannerDone = false;
            let doneDebounce = null;
            const DONE_DELAY = 1500; // 等 1.5s 確認不再更新
            const PERMISSION_TIMEOUT = 600000;
            const startTime = Date.now();
            ...
            if (messageReadyPromise) {
                messageReadyPromise.then(() => {
                    messageIsSent = true;
                    responseText = ''; // 清除 replay 文字
                    console.log('  📨 [agentStream] Message sent, accepting new responses');
                });
            }
            ...
            function checkTimeout() {
                if (done) return;
                if (permissionPending && Date.now() - startTime < PERMISSION_TIMEOUT) {
            ...
            const streamHandle = streamFetch(port, csrfToken, 'StreamAgentStateUpdates', {
                conversationId: getCascadeId(),
            }, (frameJson) => {
                // onFrame callback
                if (done) return;
                try {
                    ...
                    for (let i = 0; i < steps.length; i++) {
                        ...
                        if (step.status === 'CORTEX_STEP_STATUS_WAITING'
                            || step.status === 9 || step.status === '9') {
                            if (!handledPermissions.has(stepIndex) && messageIsSent && onPermission) {
                                handledPermissions.add(stepIndex);
                                permissionPending = true;

                                // 判斷權限類型
                                let permType = 'browser';
                                const stepType = step.type || '';
                                if (stepType.includes('TERMINAL') || stepType.includes('RUN_COMMAND') || stepType === 'CORTEX_STEP_TYPE_TOOL_RESULT') {
                                    permType = 'run_command';
                                } else if (stepType.includes('FILE')) {
                                    permType = 'file';
                                } else if (stepType.includes('MCP')) {
                                    permType = 'mcp';
                                }
                                ...
                                const trajectoryId = update.trajectoryId || getCascadeId();
                                console.log(`  🔐 [agentStream] WAITING: step ${stepIndex}, type=${stepType}`);
                                onPermission({ type: permType, trajectoryId, stepIndex, path, cmd });
                            }
                        }

                        // 權限已被處理
                        if (handledPermissions.has(stepIndex)
                    ...
                    // === 完成判斷: status IDLE + planner DONE ===
                    if (lastStatus === 'CASCADE_RUN_STATUS_IDLE' && plannerDone && responseText && messageIsSent) {
                        if (doneDebounce) clearTimeout(doneDebounce);
                        doneDebounce = setTimeout(() => {
                            console.log(`  ✅ [agentStream] Done (${responseText.length} chars)`);
                            finish(responseText);
                        }, DONE_DELAY);
                    }
                } catch (e) {
                    console.error(`  [agentStream] Frame parse error: ${e.message}`);
                }
            }, (err) => {
                // onEnd callback
                if (done) return;
                if (err) {
                    console.error(`  [agentStream] Error: ${err.message}`);
                    if (!responseText) {
                        done = true;
                        clearTimeout(timeoutHandle);
                        reject(err);
                ...
                if (responseText) {
                    console.log(`  [agentStream] Stream ended (${responseText.length} chars)`);
                    finish(responseText);
                }
            });
...
export function createRpcClient(connection, options = {}) {
    ...
    let useAgentStream = true; // try new AgentStateUpdates first
    let activeStream = null; // current stream handle for cancel
    let currentAbort = null; // AbortController for cancel support
    ...
    function streamResponse(timeoutMs, onUpdate, onPermission, messageReadyPromise = null, abortSignal = null) {
        return new Promise((resolve, reject) => {
            let responseText = '';
            let done = false;
            ...
            function checkTimeout() {
                ...
                if (responseText) {
                    console.log(`  ⏱ Timeout with partial text (${responseText.length} chars)`);
                    resolve(responseText + '\n\n⚠ *回應可能不完整（AI 回應超時）*');
                } else {
                    reject(new Error('AI response timeout'));
                }
            ...
            activeStream = streamFetch(port, csrfToken, 'StreamCascadeReactiveUpdates', {
                ...
                if (err) {
                    ...
                    // 有部分文字，帶警告送出
                    console.log(`  ⏱ Stream error with partial text (${responseText.length} chars)`);
                    finish(responseText + '\n\n⚠ *回應可能不完整（連線中斷）*');
                    return;
                }
                // Stream ended cleanly — return whatever we have
                if (responseText) {
                    console.log(`  Stream done (${responseText.length} chars)`);
                    finish(responseText);
                }
    ...
    return {
        ...
        resetCascade() {
            ...
            cascadeStarted = false;
            isFirstMessage = true;
            useAgentStream = true; // 每次新對話重試 AgentStateUpdates
        ...
        async sendToAI(message, model, onUpdate = null, timeoutMs = 300000, mediaItems = null, memoryContext = '', onPermission = null) {
            ...
            // 每次呼叫建立新的 AbortController，供 /cancel 使用
            ...
            try {
                if (useAgentStream) {
                    // === 方式 1: StreamAgentStateUpdates（最佳，即時串流）===
                    try {
                        const agent = createAgentStream(port, csrfToken, () => cascadeId);
                        let signalMessageSent;
                        const messageReadyPromise = new Promise(r => { signalMessageSent = r; });
                        const streamPromise = agent.agentStreamResponse(timeoutMs, onUpdate, onPermission, messageReadyPromise, signal);
                        streamPromise.catch(() => {});
                        ...
                        if (agentErr.message === 'Cancelled') throw agentErr;
                        console.warn(`  AgentStream failed: ${agentErr.message}, falling back to polling`);
                ...
                if (!messageSent) {
                    ...
                    console.log('  Message already sent, polling for response...');
                }
                const fallback = createPollingFallback(port, csrfToken, () => cascadeId);
                const snap = await fallback.getTrajectorySnapshot();
                return await fallback.pollResponse(timeoutMs, onUpdate, snap.executorCount, snap.lastResponseText, onPermission, signal);
            } catch (err) {
                // 取消的不重試
                ...
                if (err.message.includes('INTERNAL') || err.message.includes('cascade')
                    ...
                    if (useAgentStream) {
                        ...
                        const retryStreamPromise = agent.agentStreamResponse(timeoutMs, onUpdate, onPermission, retryReadyPromise, signal);
                        retryStreamPromise.catch(() => {});
                        await sendMessage(message, model, mediaItems, memoryContext);
                        signalRetrySent();
                        ...
                        let signalRetrySent;
                        const retryReadyPromise = new Promise(r => { signalRetrySent = r; });
                        const retryStreamPromise = streamResponse(timeoutMs, onUpdate, onPermission, retryReadyPromise, signal);
                        retryStreamPromise.catch(() => {});
                        await sendMessage(message, model, mediaItems, memoryContext);
                        signalRetrySent();
                        return await retryStreamPromise;
        ...
        async cancelCascade() {
            ...
            try {
                // 觸發 AbortController，中斷 streaming/polling
